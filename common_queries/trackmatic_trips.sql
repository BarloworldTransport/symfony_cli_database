...
`udo_fleet`.`name` AS `Primary Fleet`, FROM_UNIXTIME(UNIX_TIMESTAMP(`udo_tripleg`.`loadingArrivalETA`)+120) AS `Loading Arrival ETA`, 
FROM_UNIXTIME(UNIX_TIMESTAMP(`udo_tripleg`.`loadingArrivalTime`)+120) AS `LoadingArrival Time`, 
FROM_UNIXTIME(UNIX_TIMESTAMP(`udo_tripleg`.`loadingStarted`)+120) AS `Loading Started`, 
FROM_UNIXTIME(UNIX_TIMESTAMP(`udo_tripleg`.`loadingFinished`)+120) AS `Loading Finished`, 
FROM_UNIXTIME(UNIX_TIMESTAMP(`udo_tripleg`.`timeLeft`)+120) AS `Time Left`, 
FROM_UNIXTIME(UNIX_TIMESTAMP(`udo_tripleg`.`offloadingArrivalETA`)+120) AS `Loading Arrival ETA`, 
FROM_UNIXTIME(UNIX_TIMESTAMP(`udo_tripleg`.`offloadingArrivalTime`)+120) AS `Offloading Arrival Time`, 
FROM_UNIXTIME(UNIX_TIMESTAMP(`udo_tripleg`.`offloadingStarted`)+120) AS `Offloading Started`, 
FROM_UNIXTIME(UNIX_TIMESTAMP(`udo_tripleg`.`offloadingCompleted`)+120) AS `Offloading Completed`,
...
`udo_cargo`.`tonsActual` AS `Tons Actual`, FROM_UNIXTIME(UNIX_TIMESTAMP(`udo_tripleg`.`time_created`)+120) AS `Time Created`,
IF(!ISNULL(`cp`.`first_name`), CONCAT(`cp`.`first_name`, ' ', `cp`.`last_name`), `cp`.`email`) AS `Created By`,
FROM_UNIXTIME(UNIX_TIMESTAMP(`udo_tripleg`.`time_last_modified`)+120) AS `Last Update Time`,
IF(!ISNULL(`lmp`.`first_name`), CONCAT(`lmp`.`first_name`, ' ', `lmp`.`last_name`), `lmp`.`email`) AS `Last Updated By`,
FROM_UNIXTIME(UNIX_TIMESTAMP(`udo_trackingload`.`requestSent`)+120) AS `Trackmatic Request Sent`,
FROM_UNIXTIME(UNIX_TIMESTAMP(`udo_trackingload`.`activated`)+120) AS `Driver Activated On`
FROM `udo_cargo`
...
WHERE IF(!ISNULL(`udo_tripleg`.`loadingArrivalTime`),`udo_tripleg`.`loadingArrivalTime`,`udo_tripleg`.`loadingArrivalETA`)>='2017-09-30 22:00:00'
